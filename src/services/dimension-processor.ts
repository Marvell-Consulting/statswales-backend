...
export const cleanUpDimension = async (dimension: Dimension) => {
  ...
  if (lookupTableId && lookupTableFilename) {
    ...
    try {
      ...
      await dataLakeService.delete(lookupTableFilename, dimension.dataset.id);
    } catch (err) {
  ...
  if (dataset.dimensions) {
    for (const dimension of dataset.dimensions) {
      if (dimension.lookupTable) {
        const dataLakeService = new DataLakeService();
        try {
          dataLakeService.delete(dimension.lookupTable.filename, dataset.id);
        } catch (error) {
          logger.warn(
            error,
...
export async function removeMeasure(dataset: Dataset) {
  ...
  if (dataset.measure) {
    if (dataset.measure.lookupTable) {
      const dataLakeService = new DataLakeService();
      try {
        dataLakeService.delete(dataset.measure.lookupTable.filename, dataset.id);
      } catch (error) {
        logger.warn(
          error,
          `Something went wrong trying to remove previously uploaded lookup table with error: ${error}`
...
export const validateDateTypeDimension = async (
  ...
  try {
    const dataLakeService = new DataLakeService();
    const fileBuffer = await dataLakeService.loadBuffer(factTable.filename, dataset.id);
    fs.writeFileSync(tempFile, fileBuffer);
